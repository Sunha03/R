...
#matrix의 1열만 char 타입으로 변경
#matrix는 한 타입으로만 선언해야 하므로 한 열의 타입을 변경해도 모든 데이터의 타입이 변경됨
dataframeMat1[,1] <- as.character(dataframeMat1[,1])
dataframeMat1

#str함수는 dataframe 내부의 데이터형을 볼 수 있는 함수
str(mydataframe)
#1열만 데이터의 타입을 다르게 정의 가능
mydataframe[,1] <- as.character(mydataframe[,1])
str(mydataframe)
mydataframe

mydataframe$X1  #$를 이용해서 열의 이름에 접근 가능
mydataframe$X2
mydataframe$X3
...
#필요한 곳에서 dataframe을 다시 amtrix로 변환
